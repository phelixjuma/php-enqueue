...
class CronExpression
{
    ...
    public function setExpression(string $value): CronExpression
    {
        ...
        $questionMarkInInvalidPart = array_key_exists(0, $split) && $split[0] === '?'
            ...
            || array_key_exists(6, $split) && $split[6] === '?';
    ...
    protected function getRunDate($currentTime = null, int $nth = 0, bool $invert = false, bool $allowCurrentDate = false, $timeZone = null): DateTime
    {
        ...
        if (isset($parts[self::DAY]) && isset($parts[self::WEEKDAY])) {
            $domExpression = sprintf('%s %s %s %s %s * %s', $this->getExpression(self::SECOND), $this->getExpression(self::MINUTE), $this->getExpression(self::HOUR), $this->getExpression(self::DAY), $this->getExpression(self::MONTH), $this->getExpression(self::YEAR));
            $dowExpression = sprintf('%s %s %s * %s %s %s', $this->getExpression(self::SECOND), $this->getExpression(self::MINUTE), $this->getExpression(self::HOUR), $this->getExpression(self::MONTH), $this->getExpression(self::WEEKDAY), $this->getExpression(self::YEAR));
